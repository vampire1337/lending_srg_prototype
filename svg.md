Я понял вашу задачу. Вы абсолютно правы: для продукта уровня Enterprise B2B простые контурные иконки смотрятся слишком легковесно. Нам нужна **тяжелая, детально проработанная векторная графика**, которая кричит «мощная инфраструктура, безопасность, финтех».

Мы уходим от стоковых фото и заменяем их на сложные, многослойные SVG-иллюстрации с эффектами стекла (glassmorphism), изометрией, глубокими тенями и сложными градиентами.

Ниже я подготовил **3 премиальных SVG-кода**. Они большие и детальные.

---

### 1. Тотальный апгрейд `tech-circuit` (Схема-анимация платформы)

Этим кодом нужно **полностью заменить** текущий `<svg>` внутри `<div className="inline-infographic tech-inline reveal">` (в файле `app/page.tsx`, секция `#technology`).
Я сделал её в разы сложнее: добавил кривые Безье, многоуровневую архитектуру, орбиты данных, светящиеся пакеты информации и интеграцию с внешними системами.

```xml
<svg aria-hidden="true" className="tech-circuit" viewBox="0 0 800 450" xmlns="http://www.w3.org/2000/svg" style="width: 100%; height: auto;">
  <defs>
  ...
  <g opacity="0.05">
    <path d="M0 50 H800 M0 150 H800 M0 250 H800 M0 350 H800" stroke="#0f222f" stroke-width="1"/>
    <path d="M100 0 V450 M300 0 V450 M500 0 V450 M700 0 V450" stroke="#0f222f" stroke-width="1"/>
  </g>
  ...
  <circle className="circuit-wave wave-1" cx="400" cy="225" r="40" stroke-width="2" />
  <circle className="circuit-wave wave-2" cx="400" cy="225" r="40" stroke-width="2" />

  <path className="circuit-line line-1" d="M 80 100 C 200 100, 250 225, 400 225" fill="none" />
  <path className="circuit-line line-2" d="M 60 225 C 180 225, 200 225, 400 225" fill="none" />
  <path className="circuit-line line-3" d="M 80 350 C 200 350, 250 225, 400 225" fill="none" />
  
  <path className="circuit-line line-4" d="M 400 225 C 550 225, 600 80, 720 80" fill="none" />
  <path className="circuit-line line-5" d="M 400 225 C 580 225, 600 225, 740 225" fill="none" />
  <path className="circuit-line line-1" d="M 400 225 C 550 225, 600 370, 720 370" fill="none" />
  
  <path className="circuit-line line-2" d="M 400 40 V 225" fill="none" />
  <path className="circuit-line line-3" d="M 400 225 V 410" fill="none" />
  ...
  <g transform="translate(480, 290)">
    <rect x="-20" y="-10" width="40" height="20" rx="4" fill="#ffffff" stroke="#00b2e0" stroke-width="1.5"/>
    <text x="0" y="3" fill="#0f222f" font-size="8" font-weight="700" text-anchor="middle">Документы</text>
  </g>
</svg>
...
### 2. Изометрическая визуализация инфраструктуры (Для Блока «Для Банков» или Главного экрана)

Этим SVG можно заменить изображение в секции `#banks` (`content.banks.media.src`). Это высокодетализированная изометрия: серверные стойки, графики роста и банковский модуль, объединенные шиной данных. Выглядит очень дорого.
...
### 3. Дашборд Андеррайтера (Для Блока «Для Страховых»)

Заменяет скучное фото башни в блоке `#insurers` (`content.insurers.media.src`). Это композиция интерфейса (UI Mockup) с элементами скоринга, защиты и графиков. Показывает "Под капотом" платформы, что очень ценится в B2B.
...
<svg viewBox="0 0 600 700" xmlns="http://www.w3.org/2000/svg" style="width: 100%; height: 100%;">
  ...
  <rect x="20" y="50" width="560" height="600" rx="30" fill="url(#brandGrad)" opacity="0.05" />
...
### Как внедрить эти SVG на ваш сайт:

1. **Для замены `tech-circuit` (Блок 7):**
Откройте `app/page.tsx`, найдите строки 123-157:
`<div className="inline-infographic tech-inline reveal" aria-label="Целостная схема платформы SRG"> ... <svg> ... </svg> </div>`
И просто замените старый `<svg>` на **Первый блок кода** из моего ответа. Анимации (классы `.circuit-line`, `.circuit-wave`) подхватятся автоматически из вашего `globals.css`.
2. **Для Изометрии и Дашборда (Блоки 5 и 6):**
Поскольку сейчас там используются компоненты `<Image src="..." />`, вам нужно будет заменить их на `<div>` с вставленным SVG-кодом.
Например, в секции `#banks`:
```tsx
<aside className="section-media reveal flex items-center justify-center bg-transparent border-none">
  {/* Вставьте сюда SVG 2 (Изометрическая визуализация) */}
</aside>

```


(Уберите серый фон `.section-media` в CSS или переопределите классы, так как эти SVG сами задают себе отличный фон и объем).

Эти векторные иллюстрации решат проблему «пустоты» и стоковости, добавив сайту вес дорогого SaaS-решения уровня Enterprise.